...
exports.handler = async (event) => {
  ...
  try {
    ...
    // Add gallery (FIXED: supports image_base64 -> uploads to Supabase Storage)
    if (action === 'add_gallery' && method === 'POST') {
      const { caption, category, image_url, image_base64 } = body;
      ...
      if (image_base64 && image_base64.startsWith('data:')) {
        ...
        if (match) {
          ...
          const slug = caption.toLowerCase().replace(/[^a-z0-9]+/g,'-').slice(0,30);
          const path = 'gallery/' + slug + '-' + Date.now() + '.' + ext;
          const { error: upErr } = await db.storage.from('gallery-images').upload(path, buf, { contentType: mime, upsert: true });
          if (!upErr) {
            const { data: pub } = db.storage.from('gallery-images').getPublicUrl(path);
            if (pub && pub.publicUrl) finalUrl = pub.publicUrl;
          } else { console.error('Gallery upload error:', upErr.message); }
        }
      }
      const { data, error } = await db.from('ica_gallery').insert({ caption, category: category || 'Events', image_url: finalUrl }).select().single();
      if (error) return cors({ success: false, error: error.message }, 400);
      return cors({ success: true, data });
    }

    // Upload principal photo to Supabase Storage (NEW)
    if (action === 'upload_principal_photo' && method === 'POST') {
      const { image_base64 } = body;
      if (!image_base64 || !image_base64.startsWith('data:')) return cors({ success: false, error: 'No image provided' }, 400);
      const match = image_base64.match(/^data:([^;]+);base64,(.+)$/);
      if (!match) return cors({ success: false, error: 'Invalid image data' }, 400);
      const mime = match[1];
      const buf = Buffer.from(match[2], 'base64');
      const ext = (mime.split('/')[1] || 'jpg').replace('jpeg','jpg');
      const path = 'principal/principal-photo-' + Date.now() + '.' + ext;
      const { error: upErr } = await db.storage.from('gallery-images').upload(path, buf, { contentType: mime, upsert: true });
      if (upErr) return cors({ success: false, error: 'Upload failed: ' + upErr.message }, 500);
      const { data: pub } = db.storage.from('gallery-images').getPublicUrl(path);
      const url = pub ? pub.publicUrl : '';
      if (url) {
        await db.from('ica_settings').upsert({ key: 'principal_photo_url', value: url }, { onConflict: 'key' });
      }
      return cors({ success: true, url });
    }
